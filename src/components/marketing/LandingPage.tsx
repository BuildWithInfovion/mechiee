...
import {
  ...
  Search, LayoutDashboard, Wrench,
} from "lucide-react";
import { useState, useEffect } from "react";

/* ─────────────────────────────────────────────────────────────────
   INSTRUMENT CLUSTER  – Speedometer + 2 mini gauges
   All maths done at module level (no hydration mismatch).
───────────────────────────────────────────────────────────────── */

const CX = 210, CY = 210, R = 158;
const START = 215, SWEEP = 290;                        // 7-o'clock start, 290° total
const KMH = 78,  MAX_KMH = 160;
const arcLen   = 2 * Math.PI * R * SWEEP / 360;       // ≈ 863
const filledAt = arcLen * KMH / MAX_KMH;              // ≈ 420
...
const TICKS = Array.from({ length: 17 }, (_, i) => {
  ...
  const inner  = toXY(deg, R - (major ? 22 : 13));
  const lp     = toXY(deg, R - 42);
  return { deg, kmh, major, label, outer, inner, lp };
});

const NEEDLE_START = toXY(START, R - 16);
...
const TESTIMONIALS = [
  ...
  { name: "Priya Menon",  city: "Hyderabad", rating: 5, text: "Battery replaced in 30 minutes outside my apartment. Mechanic checked wiring for free too. Highly recommended.", bike: "TVS Jupiter", initials: "PM", bg: "bg-emerald-700" },
...
function Speedometer() {
  return (
    <svg viewBox="0 0 420 420" fill="none" xmlns="http://www.w3.org/2000/svg"
      className="w-full max-w-[420px] select-none" aria-hidden="true">
      <defs>
        {/* Active arc gradient */}
      ...
      {/* ── Background track ── */}
      <path d={arcD(R, START, SWEEP)}
        stroke="#111128" strokeWidth="28" strokeLinecap="round" fill="none" />
      <path d={arcD(R, START, SWEEP)}
        stroke="#0D0D22" strokeWidth="24" strokeLinecap="round" fill="none" />
      <path d={arcD(R, START, SWEEP)}
        stroke="#1A1A36" strokeWidth="10" strokeLinecap="round" fill="none" />

      {/* ── Active arc (animates in) ── */}
      <path
        ...
        strokeDashoffset={`${(arcLen - filledAt).toFixed(1)}`}
        style={{ animation: "arc-fill 2.8s cubic-bezier(.22,1,.36,1) forwards" }}
      />
      ...
      {/* ── Needle (animates in via rotation) ── */}
      <g
        filter="url(#glow)"
        style={{
          transformOrigin: `${CX}px ${CY}px`,
          animation: "needle-sweep 2.8s cubic-bezier(.22,1,.36,1) forwards",
        }}>
        {/* Needle body */}
        <line
          x1={CX} y1={CY}
        ...
        <line
          x1={CX} y1={CY}
          x2={NEEDLE_START.x} y2={NEEDLE_START.y}
          stroke="#F59E0B" strokeWidth="8" strokeLinecap="round" strokeOpacity="0.15"
        />
      </g>
      ...
      <text x={CX} y={CY + 32}
        ...
        fontSize="56" fontWeight="800" fontFamily="Inter,system-ui,sans-serif">
        {KMH}
      </text>
      <text x={CX} y={CY + 54}
        textAnchor="middle" fill="rgba(255,255,255,0.38)"
        fontSize="13" letterSpacing="3" fontFamily="Inter,system-ui,sans-serif">
        km/h
      </text>
      <text x={CX} y={CY - 50}
        textAnchor="middle" fill="rgba(167,139,250,0.50)"
        fontSize="10" letterSpacing="5" fontWeight="700" fontFamily="Inter,system-ui,sans-serif">
        MECHIEE
      </text>

      {/* ── Mini gauge: RPM (bottom-right) ── */}
      <MiniGauge
        x={355} y={340} r={48}
        arcColor="#7C3AED" arcOpacity="0.7"
        label="RPM" value="3.2k"
        rotClass="mini-needle"
      />

      {/* ── Mini gauge: Fuel (bottom-left) ── */}
      <MiniGauge
        x={65} y={340} r={48}
        arcColor="#10B981" arcOpacity="0.8"
        label="FUEL" value="3/4"
        rotClass="mini-needle2"
      />
    </svg>
  );
}
...
/* ─────────────────────────────────────────────────────────────────
   MINI GAUGE  (self-contained: coords relative to its own center)
───────────────────────────────────────────────────────────────── */

function MiniGauge({
  x, y, r, arcColor, arcOpacity, label, value, rotClass,
}: {
  x: number; y: number; r: number;
  arcColor: string; arcOpacity: string;
  label: string; value: string;
  rotClass: string;
}) {
  const ms = 200, me = 500;  // start/end angles for mini arc
  const mSweep = me - ms;    // 300°
  const mLen   = 2 * Math.PI * r * mSweep / 360;

  function mXY(deg: number, radius: number) {
    const rad = (deg - 90) * Math.PI / 180;
    return { x: +(x + radius * Math.cos(rad)).toFixed(1), y: +(y + radius * Math.sin(rad)).toFixed(1) };
  }

  function mArc(radius: number, a1: number, sweep: number) {
    const a2 = a1 + sweep;
    const p1 = mXY(a1, radius);
    const p2 = mXY(a2, radius);
    const la = sweep > 180 ? 1 : 0;
    return `M${p1.x},${p1.y} A${radius},${radius} 0 ${la},1 ${p2.x},${p2.y}`;
  }

  const needleTip = mXY(ms, r - 12);  // needle drawn at start; CSS rotates it
  ...
  return (
    <g>
      ...
      {/* Track */}
      <path d={mArc(r, ms, mSweep)}
        stroke="#111128" strokeWidth="10" strokeLinecap="round" fill="none" />
      {/* Active arc */}
      <path d={mArc(r, ms, mSweep)}
        stroke={arcColor} strokeWidth="5" strokeLinecap="round" fill="none"
        strokeOpacity={arcOpacity}
        strokeDasharray={`${(mLen * 0.28).toFixed(1)} ${mLen.toFixed(1)}`}
        style={{ animation: `${rotClass} 2.8s ease-out forwards` }}
      />
      {/* Needle */}
      <g style={{ transformOrigin: `${x}px ${y}px`, animation: `${rotClass} 2.8s cubic-bezier(.22,1,.36,1) forwards` }}>
        <line x1={x} y1={y} x2={needleTip.x} y2={needleTip.y}
          stroke="#F59E0B" strokeWidth="2.5" strokeLinecap="round" />
      </g>
      {/* Hub */}
      <circle cx={x} cy={y} r="7"  fill="#090918" stroke="#1E1E3C" strokeWidth="1.5" />
      <circle cx={x} cy={y} r="3"  fill="#F59E0B" />
      {/* Labels */}
      <text x={x} y={y + 20} textAnchor="middle" fill="rgba(255,255,255,0.65)"
        fontSize="11" fontWeight="700" fontFamily="Inter,system-ui,sans-serif">{value}</text>
      <text x={x} y={y + 30} textAnchor="middle" fill="rgba(255,255,255,0.32)"
        fontSize="7" letterSpacing="2" fontFamily="Inter,system-ui,sans-serif">{label}</text>
    </g>
  );
}
    ...
    <div className="min-h-screen bg-[#07070F] text-white overflow-x-hidden">
      ...
      <header className={`sticky top-0 z-50 transition-all duration-300 ${
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-[68px] flex items-center justify-between gap-6">
          ...
          {/* Logo — force white with brightness filter */}
      ...
      <section className="relative min-h-[92vh] flex items-center overflow-hidden">
        ...
        {/* Single large violet studio glow */}
        <div className="absolute inset-0 pointer-events-none" style={{
          background: "radial-gradient(ellipse 70% 65% at 62% 42%, rgba(109,40,217,0.22) 0%, transparent 65%)"
        }} />

        {/* Warm amber accent – workshop light from below-right */}
        <div className="absolute inset-0 pointer-events-none" style={{
          background: "radial-gradient(ellipse 45% 40% at 78% 80%, rgba(251,146,60,0.07) 0%, transparent 55%)"
        }} />
        ...
        {/* ─ LARGE BACKGROUND TIRE RINGS (CSS only, no SVG) ─ */}
        <div className="absolute right-0 top-1/2 pointer-events-none hidden lg:block"
          ...
          {[0,22.5,45,67.5,90,112.5,135,157.5].map((a) => (
            <div key={a} className="absolute"
              style={{
                width: "100%", height: 2, backgroundColor: "rgba(60,60,100,0.12)",
                top: "50%", left: 0,
                transform: `rotate(${a}deg)`, transformOrigin: "50% 50%",
              }} />
          ))}
          {/* Hub circle */}
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-2xl mb-16">
            ...
            <h2 className="text-4xl md:text-5xl font-extrabold text-white mb-4 leading-tight">
              ...
              <span className="text-white/30">in under 2 minutes.</span>
            </h2>
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-14">
            <div>
              ...
              <h2 className="text-4xl md:text-5xl font-extrabold text-white leading-tight">
                ...
                <span className="text-white/30">zero surprises.</span>
              </h2>
            </div>
      ...
      <section id="why-us" className="py-20 md:py-28 relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-2xl mb-16">
            ...
            <p className="text-white/42 text-lg">Every decision we make is designed around giving you a stress-free, transparent service experience.</p>
          </div>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-2xl mb-14">
            ...
            <h2 className="text-4xl md:text-5xl font-extrabold text-white leading-tight">
              ...
              <span className="text-white/30">Ask our customers.</span>
            </h2>
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div>
              ...
              <h2 className="text-4xl md:text-5xl font-extrabold text-white mb-6 leading-tight">
                ...
                <span className="text-white/30">Grow your business.</span>
              </h2>
      ...
      <section className="relative py-24 md:py-32 overflow-hidden">
        ...
        {/* Large abstract concentric rings in BG */}
        <div className="absolute -right-24 top-1/2 -translate-y-1/2 pointer-events-none hidden md:block" style={{ width: 520, height: 520 }}>
          ...
          <div className="absolute rounded-full" style={{ inset: 78, border: "1px solid rgba(255,255,255,0.04)" }} />
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-10 mb-14">
            <div className="col-span-2">
              ...
              <div className="space-y-3">
                <a href="tel:+919876543210" className="flex items-center gap-2.5 text-sm text-white/35 hover:text-white transition-colors">
                  <Phone className="w-4 h-4 text-primary" /> +91 98765 43210
                </a>
                <a href="https://wa.me/919876543210" className="flex items-center gap-2.5 text-sm text-white/35 hover:text-white transition-colors">
            ...
            <div>
              ...
              <ul className="space-y-3">
                {SERVICES.map((s) => (
                  <li key={s.label}><Link href="/login" className="text-sm text-white/30 hover:text-white transition-colors">{s.label}</Link></li>
                ))}
              </ul>
            </div>
            <div>
              ...
              <ul className="space-y-3">
                {[["About Us","#"],["For Garages","#for-garages"],["Partner Program","#"],["Careers","#"],["Blog","#"],["Contact","#"]].map(([l,h]) => (
            ...
            <div>
              ...
              <ul className="space-y-3">
                {["Privacy Policy","Terms of Service","Refund Policy","Cookie Policy"].map((l) => (
                  <li key={l}><Link href="#" className="text-sm text-white/30 hover:text-white transition-colors">{l}</Link></li>
                ))}
              </ul>
              <div className="mt-8">
                ...
                <div className="flex flex-wrap gap-2">
                  {["UPI","Cards","Net Banking","Cash"].map((p) => (
                    <span key={p} className="text-xs bg-white/[0.04] border border-white/[0.07] px-3 py-1 rounded-lg text-white/35">{p}</span>
                  ))}
                </div>
              </div>
            </div>
